...
export async function handleCreateAgent(request: Request): Promise<Response> {
  try {
    const user = await requireAuth(request);
...
export const Route = createFileRoute("/api/agents")({
  server: {
    handlers: {
      GET: ({ request }: { request: Request }) => handleListAgents(request),
      POST: ({ request }: { request: Request }) => handleCreateAgent(request),
    },
  },
});
